...
## 코드  
#### 1) GameView.java
프레임워크로서 게임 배경에 대한 그림을 비트맵으로 그리며 게임 플레이어의 조작, gameview의 surface에 대한 스레드를 관리합니다.
```java
...
등장하는 3가지 경로 중에 나오는 곳을 정해주는 m_x, 내려오는 속도를 정해주는 m_y로 존재하며 이 두 가지로 섞어 패턴을 구현했습니다.
```java
//Enemy.java
 void Move(){
